...
const FEATURES: Feature[] = [
  {label: '/api/owners', state: 'ok', breaksAt: 34},
  {label: '/api/visits', state: 'edited'},
  {label: '/api/pettypes', state: 'ok', breaksAt: 44},
  {label: '/api/vets', state: 'ok'},
  {label: '/api/pets', state: 'ok', breaksAt: 54},
  {label: '/api/specialties', state: 'ok'},
];

export const P2Blast: React.FC = () => {
  const frame = useCurrentFrame();
  const {fps} = useVideoConfig();
  return (
    <AbsoluteFill>
      <Camera duration={PS.blast} zoom={0.028}>
        <Stage gap={28}>
