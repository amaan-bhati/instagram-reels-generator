...
export const B9Verdict: React.FC = () => {
  const frame = useCurrentFrame();
  const {fps} = useVideoConfig();
  return (
    <AbsoluteFill>
      <Camera duration={BR.verdict} zoom={0.02} origin="50% 44%">
        <Stage gap={20}>
          ...
          <div style={{...enter(frame, fps, 88), marginTop: 4}}>
            <Chip label="fix it yourself, or hand it to your AI" tone="brand" size={27} />
          </div>
        </Stage>
      </Camera>
      <Caption opacity={enter(frame, fps, 102).opacity}>
