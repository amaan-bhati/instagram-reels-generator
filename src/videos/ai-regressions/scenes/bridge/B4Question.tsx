 ...
 */
export const B4Question: React.FC = () => {
  const frame = useCurrentFrame();
  const {fps} = useVideoConfig();
  return (
    <AbsoluteFill>
      <Camera duration={BR.question} zoom={0.03}>
        <Stage gap={30}>
          <div style={enter(frame, fps, 0)}>
            <Heading size={62}>So how do you stop that?</Heading>
          </div>
          <div style={{...enter(frame, fps, 30), marginTop: 6}}>
            <GradText size={54}>
      ...
      <Caption opacity={enter(frame, fps, 58).opacity}>Before your users do.</Caption>
    </AbsoluteFill>
  );
};
