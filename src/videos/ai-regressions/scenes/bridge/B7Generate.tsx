...
const NORMAL: {method: M; at: number; bars: number[]}[] = [
  {method: 'POST', at: 24, bars: [150, 92]},
  {method: 'GET', at: 30, bars: [128, 78]},
  {method: 'PUT', at: 36, bars: [140, 106]},
];
const EDGE: {method: M; at: number; bars: number[]}[] = [
  {method: 'DELETE', at: 42, bars: [112, 88]},
  {method: 'GET', at: 48, bars: [158, 72]},
  {method: 'PATCH', at: 54, bars: [124, 100]},
];
...
export const B7Generate: React.FC = () => {
  const frame = useCurrentFrame();
  const {fps} = useVideoConfig();
  const b = breathe(frame, fps, 1.5);
  return (
    <AbsoluteFill>
      <Camera duration={BR.generate} zoom={0.022} origin="50% 46%">
        <Stage gap={20}>
          ...
          <div style={{display: 'flex', gap: 20, alignItems: 'flex-start', marginTop: 2}}>
            <div style={{display: 'flex', flexDirection: 'column', gap: 13, width: 430}}>
              <ColHead label="NORMAL CASES" at={14} />
              {NORMAL.map((c, i) => (
                <TestCaseCard key={i} method={c.method} at={c.at} bars={c.bars} width={430} />
              ))}
            </div>
            <div style={{display: 'flex', flexDirection: 'column', gap: 13, width: 430}}>
              <ColHead label="EDGE CASES" brand at={18} />
              {EDGE.map((c, i) => (
                <TestCaseCard key={i} method={c.method} at={c.at} bars={c.bars} width={430} />
              ))}
            </div>
