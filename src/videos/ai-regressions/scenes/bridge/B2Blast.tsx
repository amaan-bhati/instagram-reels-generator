...
const FEATURES: Feature[] = [
  {label: '/api/owners', state: 'ok', breaksAt: 40},
  {label: '/api/visits', state: 'edited'},
  {label: '/api/pettypes', state: 'ok', breaksAt: 50},
  {label: '/api/vets', state: 'ok'},
  {label: '/api/pets', state: 'ok', breaksAt: 60},
  {label: '/api/specialties', state: 'ok'},
];

export const B2Blast: React.FC = () => {
  const frame = useCurrentFrame();
  const {fps} = useVideoConfig();
  return (
    <AbsoluteFill>
      <Camera duration={BR.blast} zoom={0.028}>
        <Stage gap={28}>
