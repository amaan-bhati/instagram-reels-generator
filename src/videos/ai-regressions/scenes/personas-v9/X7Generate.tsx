...
export const X7Generate: React.FC = () => {
  const frame = useCurrentFrame();
  const {fps} = useVideoConfig();
  const b = breathe(frame, fps, 1.5);
  return (
    <AbsoluteFill>
      <Camera duration={PV9.generate} zoom={0.018} origin="50% 44%">
        <Stage gap={18}>
          <div style={{...enter(frame, fps, 0), marginBottom: 0}}>
            <Heading size={44}>
              Keploy writes the tests and the edge
              <br />
              cases from what it learned about your app.
            </Heading>
          </div>
      ...
      <Caption opacity={enter(frame, fps, 126).opacity}>
        AI would have missed every one of these.
      </Caption>
    </AbsoluteFill>
  );
};
