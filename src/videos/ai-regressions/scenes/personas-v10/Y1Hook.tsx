 ...
 */
export const Y1Hook: React.FC = () => {
  const frame = useCurrentFrame();
  const {fps} = useVideoConfig();
  return (
    <AbsoluteFill>
      <Camera duration={AT.hook} zoom={0.03}>
        <Stage gap={24}>
          ...
          <div style={{display: 'flex', flexDirection: 'column', gap: 15}}>
            ...
            <div style={enter(frame, fps, 12)}>
              <PipelineStep
                ...
                startAt={42}
                fillFrames={52}
                detail={<DotRow n={12} startAt={44} step={4} />}
              />
            </div>
      ...
      <Caption opacity={enter(frame, fps, 96).opacity}>Everything looked fine.</Caption>
    </AbsoluteFill>
  );
};
