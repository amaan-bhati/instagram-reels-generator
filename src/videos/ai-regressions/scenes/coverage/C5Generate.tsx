...
const NORMAL: {method: M; at: number; bars: number[]}[] = [
  {method: 'POST', at: 26, bars: [150, 92]},
  {method: 'GET', at: 32, bars: [128, 78]},
  {method: 'PUT', at: 38, bars: [140, 106]},
];
const EDGE: {method: M; at: number; bars: number[]}[] = [
  {method: 'DELETE', at: 44, bars: [112, 88]},
  {method: 'GET', at: 50, bars: [158, 72]},
  {method: 'PATCH', at: 56, bars: [124, 100]},
];
...
export const C5Generate: React.FC = () => {
  const frame = useCurrentFrame();
  const {fps} = useVideoConfig();
  const b = breathe(frame, fps, 1.5);
  return (
    <AbsoluteFill>
      <Camera duration={CV.generate} zoom={0.022} origin="50% 46%">
        <Stage gap={20}>
          ...
          <div style={{...enter(frame, fps, 96), marginTop: 6}}>
